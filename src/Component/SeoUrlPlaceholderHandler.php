...
class SeoUrlPlaceholderHandler implements SeoUrlPlaceholderHandlerInterface
{
    ...
    private function createSeoMapping(SalesChannelContext $context, array $mapping): array
    {
        ...
        $mappingList = array_values($mapping);
        $mappingCount = \count($mappingList);

        $query->from('seo_url');
        $query->andWhere('seo_url.is_canonical = 1');
        $query->andWhere('seo_url.path_info IN (' . implode(',', array_fill(0, $mappingCount, '?')) . ')');
        $query->andWhere('seo_url.language_id = ?');
        $query->andWhere('seo_url.sales_channel_id = ? OR seo_url.sales_channel_id IS NULL');
        $query->andWhere('seo_url.is_deleted = 0');

        foreach ($mappingList as $index => $pathInfo) {
            $query->setParameter($index, $pathInfo);
        }

        $query->setParameter($mappingCount, Uuid::fromHexToBytes($context->getContext()->getLanguageId()));
        $query->setParameter($mappingCount + 1, Uuid::fromHexToBytes($context->getSalesChannelId()));
