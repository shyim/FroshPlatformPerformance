...
use Shopware\Core\Checkout\Cart\CartRuleLoader as CoreService;
...
class CartRuleLoader extends CoreService
{
    ...
    public function __construct(
        ...
        private readonly EntityCacheKeyGenerator $cacheKeyGenerator
    ) {
    }
    ...
    public function loadByToken(SalesChannelContext $context, string $cartToken): RuleLoaderResult
    {
        try {
            ...
            $salesChannelHash = $this->cacheKeyGenerator->getSalesChannelContextHash($context);
            $key = 'cart-rule-ids-' . $salesChannelHash;

            $cart = $this->cartFactory->createNew($cartToken);
            $ruleIds = $this->cache->get($key, function (CacheItem $item) use ($context, $cart) {
                $this->load($context, $cart, new CartBehavior($context->getPermissions()), true);

                $item->tag(['cart-rule-ids']);
            ...
            foreach ($ruleIds as $ruleId) {
                $ruleCollection->set($ruleId, $rules->get($ruleId));
            }

            return new RuleLoaderResult($cart, $ruleCollection);
        }
    }
